...
export interface SpectatorReplayPlayer {
  id: string;
  name: string;
  shirt_number: number;
  position: string;
...
export interface SpectatorReplayFrame {
  minute: number;
  phase: string;
  possession: "Home" | "Away";
  ball_zone: string;
  ball_x: number;
  ball_y: number;
  players: Record<string, { x: number; y: number; active: boolean }>;
  events: MatchEvent[];
  home_score: number;
  away_score: number;
}
...
export interface SpectatorReplayMetadata {
  ...
  players: SpectatorReplayPlayer[];
}

export interface SpectatorReplayChunk {
  chunk_number: number;
  frames: SpectatorReplayFrame[];
}
