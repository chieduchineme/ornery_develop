...
export default function MainMenu() {
  ...
  const handleStartGame = async () => {
    ...
    try {
      ...
      console.log("Failed to start match:", worldSource);
      
    if (selectedWorldId === "random") {
      // pick a random database entry that has a valid path
      const validDbs = worldDatabases.filter((db) => db.path);

      if (validDbs.length === 0) {
        console.log("No valid world databases found.");
        worldSource = undefined;
      } else {
        const randomDb =
          validDbs[Math.floor(Math.random() * validDbs.length)];

        worldSource = `file:${randomDb.path}`;
      }
    } else {
